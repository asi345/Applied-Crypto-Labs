...
class StrangeCBC():
    ...
    def encrypt(self, plaintext: bytes):
        ...
        count, i = len(plaintext) // 16, 0
        arr = self.integer.to_bytes(16, 'big')
        print(plaintext)
        ready = bytes()
        while i < count:
            cur = plaintext[i * 16 : (i + 1) * 16]
            ready += bytes([cur[i] ^ arr[i] for i in range(16)])
            i += 1
        cipher = AES.new(self.key, AES.MODE_CBC, iv=self.iv)
        ciphertext = cipher.encrypt(ready)
        return ciphertext
    ...
    def decrypt(self, ciphertext: bytes):
        ...
        """
        cipher = AES.new(self.key, AES.MODE_CBC, iv=self.iv)
        ready = cipher.decrypt(ciphertext)
        count, i = len(ready) // 16, 0
        arr = self.integer.to_bytes(16, 'big')
        plaintext = bytes()
        while i < count:
            cur = ready[i * 16 : (i + 1) * 16]
            plaintext += bytes([cur[i] ^ arr[i] for i in range(16)])
            i += 1
